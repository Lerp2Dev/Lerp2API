...
public class OnlyInRangeManager : MonoBehaviour
{
    ...
    [SerializeThis]
    private HashSet<InRange> rangedItems = new HashSet<InRange>();

    //Items being hidden
    [SerializeThis]
    private HashSet<InRange> hideList = new HashSet<InRange>();
    ...
    //Hide an item
    private IEnumerator HideItem(InRange item)
    {
    ...
    //Load an item
    private IEnumerator ViewItem(InRange item)
    {
        //Check for the file
        if (!File.Exists(Application.persistentDataPath + "/" + item.id + ".dat"))
        {
