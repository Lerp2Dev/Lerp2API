        ...
        try
        {
            Loom.QueueOnMainThread(() =>
            {
                foreach (var routine in Running)
                    ...
                    {
                        Debug.LogError("Problem starting radical coroutine " + e.ToString());
                    }
            }, 0.02f);
        }
        catch (Exception e)
        {
            Debug.LogError("Problem queing restart for radical routines " + e.ToString());
        }
    }

    #endregion IDeserialized implementation
}